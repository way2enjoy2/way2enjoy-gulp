...
describe('way2enjoy', function() {
	...
	describe('#init', function() {
		...
		it('throws error on missing API key', function() {
			expect(Way2enjoy).to.throw(Error, /missing api key/i);
		});
	});
...
describe('way2enjoy gulp', function() {
	var target = cwd + '/assets/tmp/image.png';

	before(function() {
		process.env.WAY2ENJOY_SIGS = true;
		process.env.WAY2ENJOY_KEY = key;
	});

	after(function() {
		process.env.WAY2ENJOY_SIGS = false;
	});
	...
	it('returns compressed files', function(done) {
		...
		var sh = spawn('node', ['node_modules/gulp/bin/gulp.js', 'way2enjoy']);
	...
	it('ignores files on the cli', function(done) {
		...
		var sh = spawn('node', ['node_modules/gulp/bin/gulp.js', 'way2enjoy', '--ignore', '*ge.png']);
	...
	it('forces files on the cli', function(done) {
		...
		hash.calc(file, function(md5) {
			...
			var sh = spawn('node', ['node_modules/gulp/bin/gulp.js', 'way2enjoy', '--force', '*ge.png']);
