...
var PLUGIN_NAME = 'way2enjoy-gulp',
    ...
    this.stream = function() {
        ...
        return (opt.parallel ? throughParallel : through).obj({maxConcurrency: opt.parallelMax}, function(file, enc, cb) {
            ...
            if(file.isBuffer()) {
                ...
                self.request(file).get(function(err, way2enjoyFile) {
                    if(err) {
                    ...
                    self.stats.total.out += way2enjoyFile.contents.toString().length;
                    ...
                    if(opt.sigFile) {
                        ...
                        if(opt.sameDest) {
                            curr.file = way2enjoyFile;
                            curr.hash = self.hash.calc(way2enjoyFile);
                        }

                        self.hash.update(curr.file, curr.hash);
                    }

                    this.push(way2enjoyFile);
                ...
                request.post({
                    ...
                    if(err) {
                        ...
                        if(!err) {
                            ...
                            } else err = new Error('Invalid Way2enjoy response object returned for ' + file.relative);
                        }
                    } else {
                        err = new Error('No content returned from Way2enjoy API for' + file.relative);
                    }
            ...
            get: function(cb) {
                ...
                self.upload(function(err, data) {
                    ...
                    self.download(data.url, function(err, data) {
                        ...
                        var way2enjoyFile = file.clone();
                        way2enjoyFile.contents = data;

                        cb(false, way2enjoyFile);
                    });
                });
